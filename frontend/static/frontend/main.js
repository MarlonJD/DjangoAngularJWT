...
AppComponent.ctorParameters = () => [
    { type: _blog_post_service__WEBPACK_IMPORTED_MODULE_2__["BlogPostService"] },
...
let UserService = class UserService {
    ...
    // Uses http.post() to get an auth token from djangorestframework-jwt endpoint
    registerUser(data) {
        this.http.post('http://127.0.0.1:8000/api/users/', JSON.stringify(data), this.httpOptions).subscribe(data => {
    ...
    // Uses http.post() to get an auth token from djangorestframework-jwt endpoint
    login(user) {
        this.http.post('http://127.0.0.1:8000/api/token/', JSON.stringify(user), this.httpOptions).subscribe(data => {
    ...
    getExpirationR() {
        ...
        return moment__WEBPACK_IMPORTED_MODULE_3__(expiresAtR);
    }
};
